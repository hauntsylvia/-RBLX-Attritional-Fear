...
export class NoiseHelper 
{
  ...
  public static GenerateTemperatureMap(
    XStart: number,
    YStart: number,
    XMax: number,
    YMax: number,
  ...
  {
    const NewMap: number[][] = [];
    const EquatorAtY: number = MapBoundaryMax / 2;
    for (let X = XStart; X < MapBoundaryMax && X < XMax; X++) {
      NewMap[X] = [];
      for (let Y = YStart; Y < MapBoundaryMax && Y < YMax; Y++) {
        Sleeper.Step();

        const DistanceFromEq = math.abs(Y - EquatorAtY);
        const Inv = EquatorAtY - DistanceFromEq;
  ...
  ): number[][] 
  {
    ...
    if (
      ...
      game.GetService("UserInputService").IsKeyDown(Enum.KeyCode.U)
    ) {
      print(XStart + ", " + YStart + " - NoiseHelper start.");
      print(XMax + ", " + YMax + " - NoiseHelper end.");
      print(
        MapBoundaryWidth + ", " + MapBoundaryWidth + " - NoiseHelper boundary."
      );
    }
    for (let X = XStart; X < MapBoundaryWidth && X < XMax; X++) {
      ...
      for (let Y = YStart; Y < MapBoundaryWidth && Y < YMax; Y++) {
        ...
        //}
        const nx = X / MapBoundaryWidth - 0.5;
        const ny = Y / MapBoundaryWidth - 0.5;
        ...
        const E0 = this.RidgedNoise(Frequency * nx, Frequency * ny, Z);
        const E1 = 0.25 * (math.noise(4 * nx, 4 * ny, Z) + 0.5);
        let E = E0 + E1;
