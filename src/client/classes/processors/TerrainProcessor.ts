...
export class TerrainProcessor extends Processor
{
    ...
    RenderTerrain (X: number, Z: number, ToX: number, ToZ: number): any
    {
        let TRequest = this.MakeRequest<TerrainRequest>(new ServerRequest<any>(Strings.TerrainStrings.TerrainHandlerRoute, Strings.TerrainStrings.GetMap, undefined));
        if (TRequest.Success && TRequest.Returned !== undefined)
        {
            let THelper = new TerrainHelper(TRequest.Returned);
            THelper.Render(X, Z, ToX, ToZ, Biomes.AllBiomes, Biomes.FallbackBiome);
		}
    }
}