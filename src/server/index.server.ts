...
let V = new Vessel(1, "Logic Air",
	[
		new Engine("Logic Vessel Part",
			"Yes, this is actually a piece of logic air.",
			[],
			new Geometry(MetricUnits.Base, 5, 5, 5, 10),
			PartType.Engine,
			game.GetService("Workspace").WaitForChild("Moodel") as Model,
			new StorageContainer([], Geometry.GetVolumeOfGeometry(MetricUnits.Base, G)),
			[], new Rate(10, MetricUnits.Kilo, 1, TimeUnits.Hour))
	],
	[
		new CrewMember(1, "Izolabella")
	]);
